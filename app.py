...
pio.templates["sky"] = go.layout.Template(
    layout=go.Layout(
...
ORI = ["N", "E", "S", "W"]
OCOL = {"S": CORAL, "W": "#E0A33C", "E": SKY, "N": "#5FA89F"}
CORR_COLS = NUM + [
    "Panel_Count",
    "Avg_Panel_Tilt_deg",
    "Projection_Depth_m",
    "Openness_Ratio",
    "Total_Shading_Area_m2",
    "Material_Volume_m3",
    TARGET,
]
...
def load_csv(n):
    return pd.read_csv(n) if os.path.exists(n) else None
...
@st.cache_data(show_spinner=False)
def load_meta():
    with open("dashboard_meta.json") as f:
        return json.load(f)
...
def predict_df(forest, frame):
    cats, order, trees, nt = (
        forest["orientation_categories"],
        forest["numeric_order"],
        forest["trees"],
        forest["n_trees"],
    )
    out = []
...
def mcard(label, value):
    st.markdown(
        f'<div class="mc"><div class="l">{label}</div><div class="v">{value}</div></div>',
        unsafe_allow_html=True,
    )
...
def vtable(headers, rows, kcol=0):
    ...
    for r in rows:
        ...
        body += f"<tr>{cells}</tr>"
    st.markdown(
        f'<table class="vt"><thead><tr>{h}</tr></thead><tbody>{body}</tbody></table>',
        unsafe_allow_html=True,
    )
...
if df is None or raw is None:
    st.error(
        "Missing data files — shading_units_dataset.csv and shading_units_raw.csv "
        "must be in the same folder as app.py."
    )
    st.stop()
SMAX = float(df[TARGET].to_numpy().max())

# --------------------------------------------------------------- header
st.markdown(
    f'<h1 style="display:inline;"><span style="background:{CORAL};color:{CORAL};'
    f'border-radius:5px;">▮▮</span> Smart Skylight</h1>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="tag">A generative shading facade + a machine-learning surrogate that predicts '
    "Solar Heat Gain live — in Grasshopper, and right here.</div>",
    unsafe_allow_html=True,
)
...
T = st.tabs(
    [
        ...
        "\U0001f4ca Analysis",
        "\u2699\ufe0f Model",
        "\U0001f3a8 Design studio",
        "\U0001f997 Grasshopper & files",
    ]
)
...
with T[0]:
    ...
    with L:
        ...
        st.markdown(
            '<div class="idea">An <b>attractor-driven triangular shading facade</b> is built in '
            ...
            "<b>instantly, while the sliders move</b>.</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            '<div class="idea" style="margin-top:13px;"><b>What we predict:</b> Solar Heat Gain '
    ...
    with R:
        a, b = st.columns(2)
        with a:
            mcard("Designs in dataset", "500")
        with b:
            mcard("Test accuracy", "R² ≈ 0.94")
        c, d = st.columns(2)
        with c:
            mcard("Features", "6 + orientation")
        with d:
            mcard("Deployed model", "Random Forest")
    ...
    st.markdown(
        '<div class="note"><b>Honest note:</b> targets come from a documented physics-style '
        "reconstruction (+4% noise), not a measured simulation — the GH data recorder exists to swap in "
        "measured values later with the identical pipeline.</div>",
        unsafe_allow_html=True,
    )
...
with T[1]:
    ...
    with a:
        mcard("Raw rows", "508")
    with b:
        mcard("Duplicates found", "4")
    with c:
        mcard("Missing / garbled cells", "13")
    with d:
        mcard("Clean rows", "500")

    with st.expander("See the raw (pre-cleaning) data"):
        if raw is not None:
            st.dataframe(raw.head(10), width="stretch", height=390)

    st.markdown(
        '<div class="idea" style="margin-top:14px;"><b>Cleansing — four auditable operations:</b> '
        ...
        "(4) drop rows outside each slider's real Min/Max.</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="idea" style="margin-top:12px;"><b>Transformation (qualitative → quantitative):</b> '
    ...
    st.markdown(
        '<div class="idea" style="margin-top:10px;"><b>Integration — deriving the targets</b> from the '
        "features, e.g.:</div>",
        unsafe_allow_html=True,
    )
    st.latex(
        r"\Omega \;=\; \mathrm{clip}\!\left[\left(0.2 + 0.8\,\tfrac{Aperture}{100}\right)\cdot"
        r"\tfrac{\theta}{90},\; 0,\; 1\right]"
    )
    st.latex(
        r"SHG \;=\; E(\text{orientation})\cdot \Omega \cdot \left(1 - 0.55\,g_{block}\right)\cdot"
...
with T[2]:
    ...
    with fc2:
        rng = st.slider("Solar Heat Gain range (kWh/m²)", 0.0, round(SMAX, 2), (0.0, round(SMAX, 2)))
    d = df.loc[df["Orientation"].isin(sel) & df[TARGET].between(*rng)]
    ...
    with dc1:
        distf = st.selectbox("Distribution of", NUM, index=2)
    with dc2:
        vsf = st.selectbox("Solar Heat Gain vs", NUM, index=5)
    g1, g2 = st.columns(2)
    with g1:
        h = px.histogram(d, x=distf, nbins=26, color_discrete_sequence=[SKY])
        h.update_layout(height=420, bargap=0.03, yaxis_title="count")
    ...
    with g2:
        s = px.scatter(
            ...
            color="Orientation",
            opacity=0.75,
            color_discrete_map=OCOL,
            category_orders={"Orientation": ["S", "W", "E", "N"]},
        )
        s.update_traces(marker=dict(size=6))
        s.update_layout(height=420)
...
with T[3]:
    ...
    st.markdown(
        '<div class="idea"><b>Protocol:</b> 80% train / 20% held-out test, with 5-fold '
    ...
    for name in ["Gradient Boosting", "Random Forest", "Decision Tree", "Linear Regression"]:
        s = cmp[name]
        label = ("⭐ " + name + " (deployed)") if name == "Random Forest" else name
        rows.append(
            [
                label,
                f'{s["mse"]:.1f}',
    ...
    st.markdown(
        '<div class="idea" style="margin-top:12px;"><b>Random Forest is deployed:</b> within noise of '
        "Gradient Boosting, but more interpretable (feature importances) and more robust for retraining on "
        "measured data later.</div>",
        unsafe_allow_html=True,
    )
    ...
    a, b, c = st.columns(3)
    with a:
        mcard("Trees", f'{M["n_trees"]}')
    with b:
        mcard("Decision nodes", f'{M["n_nodes"]:,}')
    with c:
        mcard("Avg tree depth", f'{M["avg_depth"]}')
    ...
    st.markdown(
        f'<div class="mono">root rule of tree #1:&nbsp; if <b>{M["root_feat"]}</b> &lt;= '
        f'{M["root_thr"]} -&gt; go left, else go right</div>',
        unsafe_allow_html=True,
    )
...
with T[4]:
    ...
    with sL:
        c1, c2 = st.columns(2)
        with c1:
            u = st.slider("U_Divisions", 1, 100, 24)
            dep = st.slider("Depth_Factor", 0.10, 1.00, 0.17)
            ap = st.slider("Aperture_Target", 0, 100, 17)
        with c2:
            v = st.slider("V_Divisions", 1, 100, 18)
            att = st.slider("Attractor_Strength", 0.00, 0.10, 0.02)
    ...
    with sR:
        ...
        st.markdown(
            '<div class="verdict"><div class="l">Predicted Solar Heat Gain</div>'
            f'<div class="v">{shg:,.1f} <small>kWh/m²</small></div></div>',
            unsafe_allow_html=True,
        )
        st.progress(min(max(shg / SMAX, 0), 1.0))
        st.markdown(
            f'<div class="sub">Hotter than {pct}% of the 500 sampled designs ' f"(dataset range 0–{SMAX:.0f}).</div>",
            unsafe_allow_html=True,
        )

    st.markdown("## The design map — your design is the dot")
    st.markdown(
        f'<div class="idea"><b>Predicted SHG across rotation × aperture — facing {o}</b> '
        "(other sliders as set above)</div>",
        unsafe_allow_html=True,
    )
    rax = np.linspace(0, 100, 21)
    aax = np.linspace(0, 100, 21)
    grid = pd.DataFrame(
        [
            {
                "U_Divisions": u,
    ...
    cm.add_trace(
        go.Scatter(
            ...
            y=[ap],
            mode="markers+text",
            marker=dict(symbol="x", size=15, color="#1B3A6B", line=dict(width=2, color="#1B3A6B")),
            text=["your design"],
    ...
    cm.update_layout(
        height=560,
        showlegend=False,
        xaxis_title="Max_Rotation_deg",
        yaxis_title="Aperture_Target",
        margin=dict(l=10, r=10, t=10, b=10),
    )
...
with T[5]:
    ...
    st.markdown(
        '<div class="idea">A Grasshopper <b>Python 3 Script component</b> wraps the trained model. '
    ...
    st.markdown(
        '<div class="idea" style="margin-top:8px;">• <span class="codepill">gh_node_predict_rhino8.py</span> '
    ...
    st.markdown(
        '<div class="idea" style="margin-top:8px;">A third component, '
    ...
    st.markdown(
        '<div class="sub" style="margin-top:8px;">Everything is downloadable on the project ' "website.</div>",
        unsafe_allow_html=True,
    )